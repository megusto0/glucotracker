...
- **Desktop + Backend** — в настройках глюкозного профиля появилась
  датированная история инсулина в помпе либо раздельных болюсного и базального
  препаратов. Для помпы один препарат покрывает базал и болюсы; можно хранить
  суточную базальную подачу, интервалы терапии не могут пересекаться, а данные
  изолированы по пользователям и не переписывают импортированные treatments.

- **Desktop + Backend** — на live-графике Nightscout маркер инсулина `I`
  открывает карточку записи. Инсулин, записанный через Glucotracker, можно
  удалить из Glucotracker и Nightscout после отдельного подтверждения;
  импортированные treatments остаются read-only и явно указывают, что их нужно
  удалять в исходном приложении.
...
- **Android (gluco) + Backend** — строки «Сон» и «Активность» на Today теперь
  ...
  совпадения минимума с пробуждением; для нагрузки — средний/пиковый пульс,
  шаги или явное «нет данных», IOB на старте и отклик в следующие два часа.
  Нераспознанную нагрузку можно назвать велосипедом, залом, ходьбой или другим
  типом и запомнить правило «ровный пульс без шагов», не изменяя исходные
  Health Connect записи. Метки owner-scoped и покрыты API-изоляцией двух
  пользователей (`b501b26`).
...
- **Desktop + Backend** — таблица автотюна называет один отрезок, который стоит
  ...
  наблюдений, а не по медиане. Карточка считает назад от начала измерения по
  текущим DIA и длительности усвоения: отдельно показывает, когда начать
  подготовку, поставить последний болюс и закончить еду, чтобы IOB и COB были
  нулевыми к тесту; базал помпы не отключается. Кнопка «Начать тест» закрыта,
  пока есть активный инсулин или углеводы. Каждый час теперь сообщает число
  различных дней — одиннадцать окон в 20:00–22:00 могут быть пятью вечерами.
...
- **Backend + Fridge** — удаление записи возвращает запасы в холодильник.
  Списание было односторонним: запись, добавленная по ошибке, уносила продукт
  с полки навсегда, поэтому каждый цикл «удалил → добавил заново» списывал одно
  и то же дважды, а контейнер милпрепа со статусом `consumed` не возвращался
  вовсе. Не хватало связи: складское движение ссылалось на лот, то есть говорило
  **что** изменилось, но не **почему**. Теперь списание несёт id приёма, а
  `POST /inventory/consume/revert` находит по нему всё, что забрала запись, и
  кладёт обратно — целиком: половину контейнера никто не съедает, а потом не
  удаляет запись об этом. Возврат записывается встречным движением, а не
  стиранием исходного: холодильник — это счёт, и счёт, забывающий свои ошибки,
  не сверить с полкой. Списания, сделанные до этой правки, откатить нельзя —
  у них нет ссылки на приём. Если холодильник недоступен, приём всё равно
  удаляется, а расхождение возвращается полем `fridge_error`.

- **Android** — одна нечитаемая строка очереди больше не останавливает всю
  очередь. `OutboxEntity.toDomain()` декодирует сохранённый тип операции и
  бросает исключение, если не может, а чтение очереди прогоняет через него
  каждую строку — поэтому одна запись, оставшаяся от старой сборки, роняла
  чтение целиком. Ни одна операция не обрабатывалась, ни у одной не росло число
  попыток, а сторож помечает «застрявшим» только то, у чего попытки были, — так
  что удаления висели в «в очереди · отправим скоро» вечно и переживали
  перезапуск, потому что строка лежит на диске. Теперь такая строка помечается
  застрявшей с понятной причиной и удаляется из «Ещё → Очередь», а остальные
  идут своим ходом.

- **Backend** — удаление записи больше не зависит от Nightscout. `DELETE /meals/{id}`
  сначала удалял зеркальную запись в Nightscout и **бросал исключение**, если та
  не отвечала, — падал весь запрос. Телефон считал это временной ошибкой, ставил
  удаление в очередь на повтор, а запись оставалась на экране: та самая «вечная
  очередь». Удаление своей записи не может быть обусловлено доступностью чужого
  сервиса. Теперь ошибка зеркала возвращается полем `mirror_error` — осиротевший
  treatment на той стороне стоит того, чтобы о нём сказать, но не того, чтобы
  из-за него отказывать в удалении.

- **Android + Backend** — отправка фото милпрепа перестала молчать. Ошибка
  уходила в лог, а на экране это выглядело ровно как «ничего не произошло».
  Кнопка показывает «Отправляем…», потом либо «Фото сохранено», либо причину:
  «Партия не найдена на сервере», «Холодильник недоступен для записи», «Нет
  связи с сервером». На сервере причины тоже разделены — «такой партии нет» и
  «файл базы недоступен для записи этому процессу» это разные проблемы с
  разными решениями, а раньше обе отдавали 404 и отправляли искать не там.
  Чтение холодильника требует только права чтения, поэтому запись фото — первый
  вызов, который может упасть на базе, с которой всё остальное работает.

- **Android + Backend** — три причины, по которым первый заход не подействовал.
  Автодополнение имело собственный маппер холодильника, и я его не тронул: коды
  `GT:C:…` попадали в названия именно оттуда, а поскольку заголовок записи
  копируется в момент создания, он переживает любую последующую правку. Клиент
  считал «своими» только пути `/photos/` и `/products/`, поэтому картинки
  холодильника (`/uploaded-media/`) и фото партии (`/fridge/…`) не приводились к
  базовому адресу и не грузились вовсе. Кнопка «Сфотографировать блюдо» жила
  только в листе поиска — теперь она и в карточке милпрепа в «Базе», куда за ней
  и приходят.

- **Android + Backend** — милпрепу можно сфотографировать блюдо с телефона.
  В листе порции у милпрепа появилась «Сфотографировать блюдо»: снимок уходит
  на `POST /fridge/mealpreps/{id}/photo` и проставляется партии и **всем её
  контейнерам** — готовят один раз, и в каждой коробке одно и то же. Клиент
  держит id контейнера, потому что это то, во что разрешается код на крышке;
  сервер сам поднимается от контейнера к партии. Отдача картинки без
  аутентификации, как у изображений продуктов, иначе мобильный Coil получает
  401 и рисует пустоту.

- **Android + Backend** — элемент приёма носит свою картинку. `MealItem.image_url`
  был вычисляемым свойством и брал изображение у связанного продукта или
  шаблона — но у запасов холодильника нет строки в `products`, их id синтетический,
  поэтому наследовать было не от чего и запись показывала заглушку. Теперь это
  настоящая колонка: клиент, который уже знает картинку, присылает её вместе с
  записью, а наследование осталось запасным вариантом.

- **Android + Backend** — граммы, остатки и картинки на местах. Строка и карточка
  в «Базе» печатают вес порции перед макросами: «300 г · Б 4,5 · Ж 22,2 · У 20» —
  без него «Б 9,1 · Ж 7,8 · У 5,5» не факт, а набор цифр, потому что неизвестно,
  ложка это или ведёрко. Карточка перестала печатать «product» и показывает
  остаток. Штуки предлагаются всегда, когда холодильник знает вес одной штуки, а
  не только когда лот считается в штуках: 0,9 кг яблок — это всё ещё яблоки по
  одному. Остаток лота приводится к граммам на сервере — килограммовый лот
  отдавал «0.9», а клиент читает это как потолок для ползунка в граммах.
  Картинка продукта попадает на «Сегодня»: запись, заведённая из «Базы», брала
  URL картинки и клала его в поле локального файла, откуда загрузчик делал
  `File("https://…")` — файла нет, запись висела в очереди, а на «Сегодня»
  стояла заглушка.

- **Android + Backend** — товар из холодильника виден как товар из холодильника.
  Метка стоит рядом с названием и в поиске, и в «Базе», а справа — остаток:
  «осталось 4 шт», «осталось 3 конт.», а за два дня до конца срока вместо
  остатка встаёт «съесть за N дн.». Штучные товары считаются штуками по
  настоящему весу штуки: раньше он терялся по дороге, и яблоко, яйцо и головка
  чеснока весили ровно по 100 г. Шесть контейнеров одной готовки схлопнулись в
  одну строку с числом оставшихся — раньше это были шесть товаров, отличавшихся
  только кодом `GT:C:…` в названии, который список всё равно обрезал.

- **Android** — «База» перестала врать в трёх местах. Бейдж на каждой строке
  печатал слово «product» — английское, одинаковое, о том, списком чего экран и
  является; макросы из-за него не помещались и обрывались на «У 7…». Счётчик
  «× 120 раз» у товаров холодильника был выдуманным числом, а не историей.
  Фильтр «Рестораны» показывал всё, у чего есть подпись, потому что сравнивал
  захардкоженное поле; «Продукты» дублировал вид по умолчанию. Остались
  «Частые», «В холодильнике», «Шаблоны» и «Требуют проверки», а запасы сортируются
  по тому, что испортится раньше, — неизвестный срок уходит вниз, а не наверх.

- **Android (gluco)** — подвал карточки приёма больше не режет обе свои
  половины. Зоны делили ширину поровну независимо от содержимого, поэтому
  «9,0 ЕД вместе с едой» выходило как «9,0 ЕД вместе с едо…», а исход обрезался
  на своей стороне вообще без многоточия: «наблюдение продолжается» доезжало до
  «наблюдение». Исход теперь занимает свою ширину, доза получает остаток, а
  строка состояния сокращена до «наблюдение» — «продолжается» не добавляло
  ничего к тому, что уже говорит время. Оба состояния подвала получили голдены.

- **Android (gluco)** — карточка с одним блюдом не печатает свои числа дважды.
  Шапка приёма несла «86,9 г · 685 ккал», а строка блюда двадцатью пикселями
  ниже — «685 ккал · 86,9 г»: те же два числа, в обратном порядке. Сумма
  остаётся только там, где суммирует больше одного блюда.

- **Android (gluco)** — эпизод без еды стал одной строкой вместо карточки. Доза
  и время печатались дважды: в шапке и в левой зоне подвала сорока пикселями
  ниже, потому что вёрстка карточки приёма разводит шапку и подвал по краям
  блюда, а без блюда между ними пусто. Осталась одна строка с теми же двумя
  зонами — слева доза, справа исход, — как у соседних строк сна и активности.
  «Nightscout» ушёл в подсказку по долгому нажатию: на каждой коррекции стоял
  факт о том, откуда пришли данные, набранный весом дозы. Строка получила свой
  голден: до сих пор её не видел ни один снимок, потому что экранные голдены
  gluco рисуют фейковую поверхность.

- **Android + Backend** — разбор эпизода показывает только то, чего нет в другом
  месте того же экрана. Плашка «как прошёл приём» несёт вердикт и больше ничего:
  старт, пик и «через 2 ч» уже стоят строками 1–3, подъём — производной, сумма —
  в шапке, а смещение каждой дозы — своей строкой, так что три строки пересказа
  повторяли две над собой. Если сказать нечего — плашки нет. Собственные болюсы
  приёма ушли из «что ещё было рядом» в одну сворачиваемую строку: это лечение
  эпизода, а не его окрестность, и четыре дозы больше не занимают четыре строки,
  три из которых говорили «вместе с едой». Буква на графике осталась только у
  соседних эпизодов — единственного, что гнёт эту кривую; сон и собственные дозы
  находить на ней незачем. «Ммоль/л на грамм» показывается только там, где
  инсулина не было: на покрытом приёме эта величина меряет совпадение болюса с
  едой под подписью, которая говорит другое.
...
- **Backend / Android (gluco)** — разбор сна и активности снова открывается из
  Today: переданный Android UTC-момент теперь переводится обратно в локальное
  wall-clock время перед поиском записи. Раньше при часовом поясе Самары поиск
  сдвигался на четыре часа, сервер отвечал 404, а приложение показывало это как
  «нет связи с сервером» (`5aa1fdf`).

- **Backend / Android (gluco)** — расчёт нового пищевого болюса всегда выдаёт
  числовой итог для доступной еды: signed-поправка по текущему прогнозу больше
  не обрезается до нуля, а свободный IOB после покрытия прежнего COB вычитается
  отдельной видимой строкой. При прогнозе ниже 3,9 итог равен 0,0 ЕД вместо
  «расчёт отложен»; текущий лист использует состояние на момент открытия, а
  ретроспективный — на момент болюса. «Разбор эпизода» различает избыточное,
  достаточное и недостаточное покрытие, поздний болюс/усвоение, подъём без
  покрытия и еду на фоне предыдущего активного инсулина (`aaf4860`).

- **Backend / Android (gluco)** — поздние болюсы больше не перегружают график
  разбора буквенными точками и вертикалями: они остаются отдельными строками со
  значком шприца и точным временем. Вывод теперь соблюдает хронологию — пишет
  «До пика болюса не было» или «Подъём начался без болюса», когда первый укол
  появился только после еды, вместо неверного «На фоне болюса» (`7c1a9da`).

- **Backend / Android (gluco)** — разбор пищевого эпизода больше не скрывает
  связанный с ним инсулин: каждый болюс показан строкой «Инсулин N ЕД» с точным
  временем относительно еды, включая поздний болюс «через 53 мин после еды»
  (`f3084e8`).
